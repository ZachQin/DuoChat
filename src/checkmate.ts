...
export function createCheckMate(executor: TypeChatLanguageModel, verifier: TypeChatLanguageModel): Checkmate {
    ...
    function createVerifyInitialPrompt(taskDescription: string, executorSolution: string): string {
        return `As the task verifier, your job is to review the solution generated by the executor based on the following task:\n${taskDescription}\nThe executor's solution is:\n${executorSolution}\nIf the solution meets the standard, please respond with: \`{"isSatisfactory": true}\`. If it does not meet the standard, please respond with: \`{"isSatisfactory": false, "feedback": "<your feedback here>"}\`.`;
    }

    function createVerifyFollowupPrompt(updatedExecutorSolution: string): string {
        return `The executor has refined their solution based on your previous feedback. The updated task solution is:\n${updatedExecutorSolution}\nAs the task verifier, please review it again. If the solution meets the standard, please respond with: \`{"isSatisfactory": true}\`. If it still does not meet the standard, please respond with: \`{"isSatisfactory": false, "feedback": "<your new feedback here>"}\`.`;
    }

    function createVerifyInitialPromptBeforeMinRefinementRounds(taskDescription: string, executorSolution: string): string {
        return `As the task verifier, your job is to review the solution generated by the executor based on the following task:\n${taskDescription}\nThe executor's solution is:\n${executorSolution}\nPlease provide feedback on the solution.`;
    }

    function createVerifyFollowupPromptBeforeMinRefinementRounds(updatedExecutorSolution: string): string {
        return `The executor has refined their solution based on your previous feedback. The updated task solution is:\n${updatedExecutorSolution}\nAs the task verifier, please review it again. Please provide feedback on the solution.`;
    }
